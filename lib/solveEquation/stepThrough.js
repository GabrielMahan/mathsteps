...
function solveConstantEquation(equation, debug, steps=[]) {
  ...
  let rNodeIsConstant = Node.Type.isOverallConstant(equation.rightNode)
  let lNodeIsConstant = Node.Type.isOverallConstant(equation.leftNode)
  if (!lNodeIsConstant) {
    console.log("left node xxx ", equation.leftNode.toString())
  }
  if (!rNodeIsConstant) {
    console.log("right node xxx ", equation.rightNode.toString())
  }
